...
```{r func, echo=FALSE, fig.cap = "A function can be thought of as a system: we have an input value $x$, the system is the function $f$, and the output is the value $f(x)$", fig.alt = "Depiction of a function as a system, which takes an input $x$ and returns an output $f(x)$."}
...
```{r, lines, echo=FALSE, fig.cap="A general line $ax+by+c=0$ (red line) a line specified by a gradient and intercept $y=mx+d$ (blue line), a vertical line $x=A$ (green line) and a horizontal line $y=B$ (orange line). Adjust the sliders to change the constants and check which values of $m,d,A,$ and $B$ correspond to $a,b$ and $c$."}
knitr::include_url("https://www.desmos.com/calculator/ao47bht5xb")
```
 <a href="https://www.desmos.com/calculator/ao47bht5xb" target="_blank">[Open preceding graph in new browser tab.]</a>
<!--
<a href="https://www.desmos.com/calculator/ao47bht5xb" target="_blank">[Open graph in new browser tab.]</a>
Open graph here: \url{https://www.desmos.com/calculator/ao47bht5xb}"
-->
...
```
<a href="https://www.desmos.com/calculator/fxiq9a83fp" target="_blank">[Open preceding graph in new browser tab.]</a>
...
```{r, asymp, echo=FALSE, fig.cap="A plot of the rational function $f(x)=\\frac{x^{2}}{x^{2}-4}$ (solid red line) and its asymptotes (dashed blue lines)."}
knitr::include_url("https://www.desmos.com/calculator/m4ssgias64")
```
<a href="https://www.desmos.com/calculator/m4ssgias64" target="_blank">[Open preceding graph in new browser tab.]</a>
...
```{r, sincos, echo=FALSE, fig.cap="The functions $\\sin(x)$ (red curve) and $\\cos(x)$ (blue dashed line)."}
knitr::include_url("https://www.desmos.com/calculator/gyj6vrtzbu")
```
<a href="https://www.desmos.com/calculator/gyj6vrtzbu" target="_blank">[Open preceding graph in new browser tab.]</a>
...
```{r, tangent, echo=FALSE, fig.cap="The function $\\tan(x)$ (green curve)."}
knitr::include_url("https://www.desmos.com/calculator/ncqey1cfus")
```
<a href="https://www.desmos.com/calculator/ncqey1cfus" target="_blank">[Open preceding graph in new browser tab.]</a>
...
```{r, exponentialfunc, echo=FALSE, fig.cap="The function $f(x)=a^x$ for varying positive values of $a$."}
knitr::include_url("https://www.desmos.com/calculator/5d0shgjfag")
```
<a href="https://www.desmos.com/calculator/5d0shgjfag" target="_blank">[Open preceding graph in new browser tab.]</a>
...
Hyperbolic sine, usually pronounced as "shine", is defined as
...
```{r, hyperbol, echo=FALSE, fig.cap="The functions $\\sinh(x)$ (red curve), $\\cosh(x)$ (blue curve) and $\\tanh(x)$ (green curve)."}
knitr::include_url("https://www.desmos.com/calculator/xrpc0in171")
```
<a href="https://www.desmos.com/calculator/xrpc0in171" target="_blank">[Open preceding graph in new browser tab.]</a>

In section \@ref(trigonometry) we will introduce several trigonometric identities. Similar identities also hold for the corresponding hyperbolic functions, except that where ever there is a $\sin^2$ we replace it with a $-\sinh^2$. For example:
...
As an example of where these functions arise in the real-world, the shape formed by a flexible cable or chain hanging from its ends under its own weight has (to a good approximation) the general form
\begin{equation*}
...
## Inverse Functions {#InvFunc}

Thinking of a function again as a system, we sometimes may be given a certain output and wish to know what the original input was. If there exists a function that returns the input leading to a given output, then we call this function the *inverse*. We generally denote the inverse of a function $f$ by $f^{-1}$. This operation is itself a function. For example, if
...
For example, $f(3)=2\times3+1=7$ and $f^{-1}(7)=0.5\times 7- 0.5 = 3$. So *inverse* functions "undo" the action of the original function to return to the original value $x$. If we first apply $f$ and then $f^{-1}$ (known as the *composition* of $f$ and $f^{-1}$) to a given input $x$, then the result is to get back to $x$. The same is also true if we first apply $f^{-1}$ and then $f$. That is:
...
HOWEVER, the inverse function $f^{-1}$ is not generally given by $f^{-1}(x)=\dfrac{1}{f(x)}$, it is more complicated than that. The notation $f^{-1}$ is used becuase it is similar in spirit to the relationship between multiplication and division.
...
and the output is $f(x)=4$, then the input could have been $x=2$ or it could have been $x=-2$. We call such functions "many-to-one" (many inputs give the same output). An extreme example is the function $f(x)=0$ for all $x$. This sends all inputs $x$ to the same output value $0$, so knowing that the ouput is $0$ doesn't give us any information about what the input $x$ was.

In order to have an inverse, our function must be "one-to-one" (each output comes from a unique input). Going back to $f(x)=x^2$, we are not completely stuck in finding an inverse. If we only consider non-negative inputs to the function, then the output does have a unique input. The inverse is then the square root function $f^{-1}(x)=\sqrt{x}$. We sometimes call such an inverse a *partial inverse*. We are just considering the inverse of half of the $y=x^2$ graph where $x$ is non-negative. We will revisit this particular example in more generality in the subsection on root functions later in this section. 
...
When we have an algebraic formula for a function there is a general procedure for finding the inverse:
...
```{r, x2m1Inv, echo=FALSE, fig.cap="The function $g(x)=x^2-1$ (red curve) and its partial inverse $g^{-1}(x)=\\sqrt{x+1}$ (blue curve) together with the line of symmetry $y=x$."}
knitr::include_url("https://www.desmos.com/calculator/l763j7dm90")
```
<a href="https://www.desmos.com/calculator/l763j7dm90" target="_blank">[Open preceding graph in new browser tab.]</a>
...
```{r, RationalInv, echo=FALSE, fig.cap="The function $h(x)=\\dfrac{x-2}{x-3}$ (red curve) and its partial inverse $h^{-1}(x)=\\dfrac{3x-2}{x-1}$ (blue curve) together with the line of symmetry $y=x$."}
knitr::include_url("https://www.desmos.com/calculator/wsktzns4ca")
```
<a href="https://www.desmos.com/calculator/wsktzns4ca" target="_blank">[Open preceding graph in new browser tab.]</a>
:::
...
### Inverse Trigonometric Functions {#InvTrigFuncs}
...
```{r, arcsin, echo=FALSE, fig.cap="The function $\\arcsin(x)$ (blue curve)."}
knitr::include_url("https://www.desmos.com/calculator/eddkyckfo5")
```
<a href="https://www.desmos.com/calculator/eddkyckfo5" target="_blank">[Open preceding graph in new browser tab.]</a>
...
```{r, arccos, echo=FALSE, fig.cap="The function $\\arccos(x)$ (blue curve)."}
knitr::include_url("https://www.desmos.com/calculator/uoqsozktti")
```
<a href="https://www.desmos.com/calculator/uoqsozktti" target="_blank">[Open preceding graph in new browser tab.]</a>
...
```{r, arctan, echo=FALSE, fig.cap="The function $\\arctan(x)$ (blue curve)."}
knitr::include_url("https://www.desmos.com/calculator/fo2mklyvjb")
```
<a href="https://www.desmos.com/calculator/fo2mklyvjb" target="_blank">[Open preceding graph in new browser tab.]</a>
...
$\ln(x)$ is usually pronounced as *lun x* or *L.N. of x*.

```{r, expln, echo=FALSE, fig.cap="The functions $f(x)=a^x$ for varying $a>1$ (red curve) and the inverse $g(x)=\\log_a(x)$ (blue curve) together with the line $y=x$ to show the symmetry of the function and its inverse."}
knitr::include_url("https://www.desmos.com/calculator/wlp8muzfqk")
```
<a href="https://www.desmos.com/calculator/wlp8muzfqk" target="_blank">[Open preceding graph in new browser tab.]</a>
...
but we should always check what the author intends as sometimes $\log(x)$ is used to mean $\ln(x)$. We have the following rules for manipulating logarithms (many of these follow from the laws of exponents in section \@ref(exponents); try to derive them yourself):
...
For any given $a, b, y > 0$,
\begin{equation*}
\log_a(y)=\dfrac{\log_b (y)}{\log_b (a)}.
\end{equation*}
:::
This change of base theorem allows us to calculate $\log_a$ of a number in terms of $\log_b$. It follows since, if $y=a^x$, then $x=\log_a(y)$. Also, $\log_b(y)=x\log_b(a)$, and so $x=\dfrac{\log_b(y)}{\log_b(a)} = \log_a(y)$.
